      ...
      <view class="form-card dk-card">
        ...
        <view class="field">
          ...
          <picker mode="time" :value="form.remindTime" @change="form.remindTime = $event.detail.value">
            <view class="input picker-row time-input">
...
const form = ref({
  ...
  repeatMonthValue: 'last_day', remindTime: '07:30', description: '', remark: '', targetCount: '', planDate: todayDefault
})
...
function submit() {
  ...
  if (!f.planDate) return uni.showToast({ title: '请选择开始打卡事项时间', icon: 'none' })
