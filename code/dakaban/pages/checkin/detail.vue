    ...
    <scroll-view class="scroll-content" scroll-y :style="{ paddingTop: (statusBarHeight + 44) + 'px' }">
      <view v-if="item._id" class="main-content">
        ...
        <view class="hero-card">
          <!-- ★ 已达成目标徽标 -->
          <view v-if="item.status === 'completed'" class="hero-badge-completed">
            <text class="badge-completed-icon">🏆</text>
            <text class="badge-completed-text">已达成目标</text>
          </view>
        ...
        <view class="action-section">
          ...
          <view class="action-list">
            ...
            <view class="action-row" @tap="togglePause">
              <text class="action-icon action-icon-pause">{{ item.status === 'paused' ? '▶' : item.status === 'completed' ? '↺' : '‖' }}</text>
              <view class="action-info">
                <text class="action-name">{{ item.status === 'paused' ? '恢复此事项' : item.status === 'completed' ? '重新激活事项' : '暂停此事项' }}</text>
                <text class="action-desc">{{ item.status === 'paused' ? '恢复后重新计算剩余次数和提醒' : item.status === 'completed' ? '激活后将重新出现在每日待打卡列表' : '暂停期间不计算剩余次数' }}</text>
              </view>
...
/** 暂停/恢复切换（含 completed → active 重新激活） */
async function togglePause() {
  const newStatus = item.value.status === 'paused' ? 'active' : item.value.status === 'completed' ? 'active' : 'paused'
  const actionName = newStatus === 'paused' ? '暂停' : (item.value.status === 'completed' ? '重新激活' : '恢复')
